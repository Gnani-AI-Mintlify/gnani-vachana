...
- **TTS `speed` parameter** — optional speaking-rate control for `timbre-v2.5` on all TTS clients (REST, SSE, WebSocket). Range `0.85`–`1.15`, default `1.0`. Exported as `DEFAULT_SPEED`, `MIN_SPEED`, and `MAX_SPEED` from `gnani.tts`.
